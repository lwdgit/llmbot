...
export async function chat(prompt: string) {
  const api = new BingChat({ debug: Boolean(process.env.NODE_DEBUG), cookie: process.env.BING_COOKIE || '' })
  const response = await api.sendMessage(prompt, ConversationState.value);
